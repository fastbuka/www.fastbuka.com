...
export default function PhoneVerification(props: Props) {
  const { asPage, phone } = props;
  const { openModal } = useAuthModal();
  const router = useRouter();
  const { loading, phoneLogin, verifyPhone } = useAuth();
  ...
  useEffect(() => {
    const savedPhone = localStorage.getItem("ACCOUNT_REGISTRATION_NUMBER");
    if (!savedPhone && !phone) {
      handleBackToLogin();
    }
    const value = asPage ? savedPhone : phone;
  ...
  return (
    <div className="w-[472px] @max-2xl:w-full max-w-full flex flex-col items-center">
      ...
      <p className="mt-2.5 mb-5 2xl:mb-6 text-center max-w-[345px] font-normal text-sm 2xl:text-base text-[#5D5D5D]">
        Please input code sent to your number
      </p>
